...
function sig_handler()
{
	...
	$grbs_tuners = $settings->grbs_tuners;
	if( $type === 'GR' ){
		$smf_type   = 'GR';
		$sql_type   = 'type="GR"';
	...
	$oth_sql_use = 'complete=0 AND '.$oth_type.' AND endtime>subtime( now(), sec_to_time('.($settings->extra_time+2).') ) AND starttime<addtime( now(), sec_to_time('.$epg_tm.') )';
	...
	while(1){
		...
		if( ($type === "GR" && $grbs_tuners > 0) || ($type === "BS" && $grbs_tuners > 0) ){
			$oth_revs       = $res_obj->fetch_array( null, null, $oth_sql_use );
...
//file_put_contents( '/tmp/debug.txt', '$off_tuners='.$off_tuners.' , $oth_off_tuners='.$oth_off_tuners."\n", FILE_APPEND );
